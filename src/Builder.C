...
dom::Element* Builder::currentElement;
dom::Element* Builder::parentElement;

Builder::Builder(dom::Document* _factory) : currentAttr(0)
{
    factory = _factory;
...
{
    currentElement->appendChild(static_cast<dom::Node *>(factory->createTextNode(trim(text))));
}
...
void Builder::createElement(const std::string& tag)
{
    ...
    if (factory->getChildNodes()->size() == 0) // This is the root element.
    {
...
void Builder::setElementAsInitialized()
{
    dynamic_cast<ElementProxy*>(currentElement)->setIsChildInitialized(true);
}

void Builder::setElementInitPoint(std::streampos pos)
{
    dynamic_cast<ElementProxy*>(currentElement)->setElementInitPoint(pos);
}

void Builder::setDirector(Director* director)
{
    dynamic_cast<ElementProxy*>(currentElement)->setDirector(director);
}
