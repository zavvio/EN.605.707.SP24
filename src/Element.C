...
void ElementProxy::parseChildNodes(void)
{
    printf("[INFO] Child node for \"%s\" being initialized on-demand!\n", this->getNodeName().c_str());
    pDirector->tokenizer.setFilePosition(streamPosition);
    pDirector->builder->setParentElement(this);
    pDirector->continueParsing();
    isChildInitialized = true;
}

void ElementProxy::serialize(std::fstream* writer, WhitespaceStrategy* whitespace)
{
    if (!isChildInitialized) parseChildNodes();
    return element->serialize(writer, whitespace);
}
...
dom::Node* ElementProxy::insertBefore(Node* newChild, Node* refChild)
{
    if (!isChildInitialized) parseChildNodes();
    return element->insertBefore(newChild, refChild);
}

dom::Node* ElementProxy::replaceChild(Node* newChild, Node* oldChild)
{
    if (!isChildInitialized) parseChildNodes();
